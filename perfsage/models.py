...
class SampleAndAggregate(GeneralizedModel):
    ...
    def sample(self, inputs, adj_info, layer_infos, batch_size=None):
        ...
        for k in range(len(layer_infos)):
            ...
            node = sampler((samples[k], layer_infos[t].num_samples, adj_info))
            samples.append(tf.reshape(node, [support_size * batch_size,]))
            support_sizes.append(support_size)
        return samples, support_sizes


    def aggregate(self, samples, input_features, input_category_lookup, input_category_embeddings, dims, num_samples, support_sizes, batch_size=None,
        ...
        category_ids = [tf.nn.embedding_lookup(input_category_lookup, node_samples) for node_samples in samples]
        category_embeddings = [tf.nn.embedding_lookup(input_category_embeddings, category_id) for category_id in category_ids]
        for i in range(len(hidden)):
            hidden[i] = tf.concat([hidden[i], category_embeddings[i]], 1)
